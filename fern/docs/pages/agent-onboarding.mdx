...
```python
# First, register a webhook to receive orders
httpx.post(
    f"{BASE}/v1/webhooks",
    headers={"Authorization": f"Bearer {api_key}"},
    json={
...
print(f"Fulfillment: {listing['fulfillment']}")
```
...
#   POST /v1/listings  { name, price, deliverable }
#   → fulfillment: "managed", readiness.sellable: true
#
# STEP 7b: External listing (your app delivers)
#   POST /v1/webhooks  { url, events: ["order.paid"] }
