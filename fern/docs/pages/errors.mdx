...
| `422` | Unprocessable entity | The request is well-formed but fails validation (e.g. negative price, unsupported currency). |
...
resp = httpx.post(
    ...
    json={"name": "My Guide", "price": -100, "currency": "usd"},
)
...
const resp = await fetch("https://api.listbee.so/v1/listings", {
  ...
  body: JSON.stringify({ name: "My Guide", price: -100, currency: "usd" }),
});
