...
Yes, but not the content. Use `PUT /v1/listings/{slug}` to update name, price, currency, description, and cover image. The `content` field is immutable — delete the listing and recreate it to change the content payload.
...
#
# Updates via PUT /v1/listings/{slug}: name, price, currency, description, cover
#
